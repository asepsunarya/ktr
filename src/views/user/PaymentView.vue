                  ...
                  <template v-if="payments.length > 0">
                    <tr v-for="(payment, index) in payments" :key="index" class="text-gray-700 dark:text-gray-400 cursor-pointer">
                      ...
                      <td class="px-4 py-3 text-sm">
                        <a :href="`/user/request/${payment.ktr_request_id}`" class="underline text-indigo-600">{{ payment.ktr_request.purpose }}</a> 
                      </td>
                      <td class="px-4 py-3 text-sm">
...
const fetchStatistics = async () => {
  const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/payments/user/${user.id}/stats`)
  statistics.value = data
}

const fetchPayments = async () => {
  const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/payments/user/${user.id}`)
  payments.value = data
}
